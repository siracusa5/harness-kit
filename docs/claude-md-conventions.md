...
Claude Code only auto-discovers files named `CLAUDE.md` (and `.claude.md`, `.claude.local.md`). It does **not** auto-load `AGENT.md` or `SOUL.md`. The global `~/.claude/CLAUDE.md` must include an explicit instruction to read the companion files:
...
Not every user needs a SOUL.md. It's relevant when Claude is used as a long-running collaborator with continuity across sessions, not just a code assistant.

---

## Transient Content

Never put transient content in CLAUDE.md. Transient content includes:
...
- **TODO.md** — for project-level task lists
- **`.claude.local.md`** — for session-local context (gitignored)
...
The three-file convention works alongside several native Claude Code features. These aren't required but are worth knowing about.

### Modular Rules Directory

For projects where even CLAUDE.md gets long, `.claude/rules/` (v2.0.64+) lets you split instructions into separate auto-loaded files:
...
Every `*.md` in this directory loads automatically alongside CLAUDE.md. Supports nested directories for scoped overrides. Good for large teams where different people own different conventions.

This complements the three-file split: CLAUDE.md stays focused on Commands/Architecture/Gotchas, while detailed rules live in `.claude/rules/`.

### Permission Modes

Four modes control what Claude can do in a session:
...
Practical tip: **start read-only, grant write access last.** When onboarding someone to a codebase, have them use `plan` mode first to explore before editing.
...
claude -p --max-turns 5 "query"                      # cap iterations
claude -p --max-budget-usd 2 "query"                 # spending limit
claude -p --json-schema '{"type":"object",...}' "q"   # validated output
```

Combine with `--output-format json` and `--json-schema` for reliable CI integrations (PR review bots, issue triage, code generation pipelines).
...
- **Limit write access** — read-only agents for analysis/review, write access only where needed.
...
MCP servers extend Claude with external tools (GitHub, databases, APIs). A few guidelines:

- **Don't add too many servers at once** — each one adds to context and affects performance.
- **Only add trusted servers**, especially those with network access.
- **Clean up unused servers** regularly with `claude mcp remove <name>`.
- **Scope appropriately** — use project scope (`.mcp.json`) for project-specific servers, user scope (`-s user`) for personal tools.
